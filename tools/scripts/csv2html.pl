...
$ver = '$Version: 2.3.dmc $'; # Douglas McClendon <dmc@ascendos.org>
##########################################################################
    ...
    foreach $field (@arecord) {   # For each output field
        ...
        if ($opt_b) {             # If we're doing bold/center
            if ($firstline) {     # And it's the first line
                ...
                print OUTFILE ("    <td style=\"white-space:nowrap;\" align=\"center\"><b>$field</b></td>\n");
            } else {              # Otherwise, make it normal
                print OUTFILE ("    <td style=\"white-space:nowrap;\ align=\"center\">$field</td>\n");
            } # end of if firstline
        } else {                  # Otherwise, make it normal
            print OUTFILE ("    <td style=\"white-space:nowrap;\" align=\"center\">$field</td>\n");
